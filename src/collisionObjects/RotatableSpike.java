...
public class RotatableSpike extends CollisionObject {
	 ...
	 * @param rotation Character representing the rotation. 'u' = up, 'd' = down,
	 *                 'l' = left, 'r' = right
	...
	/**
	 * Checks if Madeline is colliding with the top of the spike. If she is, it
	 * kills her.
	 */
	@Override
	public boolean isCollidingFloor(int madelineX, int madelineY) {
		if (super.isCollidingFloor(madelineX, madelineY) && canDie()) {
			m.death();
			return true;
		}
		return false;
	...
	/**
	 * Checks if Madeline is colliding with the side of the spike. If she is, it
	 * kills her.
	 */
	@Override
	public boolean isCollidingWall(int madelineX, int madelineY, int facing) {
		if (super.isCollidingWall(madelineX, madelineY, facing) && canDie()) {
			m.death();
			return true;
		}
		return false;
	...
	/**
	 * Checks if Madeline is colliding with the bottom of the spike. If she is, it
	 * kills her.
	 */
	@Override
	public boolean isCollidingCeiling(int madelineX, int madelineY) {
		if (super.isCollidingCeiling(madelineX, madelineY) && canDie()) {
			m.death();
			return true;
		}
		return false;
	...
	/**
	 * Checks if Madeline can die or not. If she is moving in the same direction of
	 * the spike, she should not die.
	 * 
	 * @return false if Madeline should not die, true if she should
	 */
	private boolean canDie() {
		if (rotation == 'd' && m.getYVelocity() > 0) {
			return false;
		} else if (rotation == 'l' && m.getXVelocity() < 0) {
			return false;
		} else if (rotation == 'r' && m.getXVelocity() > 0) {
			return false;
		} else if (rotation == 'u' && m.getYVelocity() < 0) {
			return false;
		}
		return true;
	}
